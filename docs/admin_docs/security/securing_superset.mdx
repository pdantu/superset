...
Superset uses Flask-Talisman to set HTTP security headers (CSP, HSTS, X-Frame-Options, etc.). Talisman is enabled by default (`TALISMAN_ENABLED = True`). If Talisman is explicitly disabled, a warning will be logged at startup.

> #### ⚠️ Important: Do Not Disable Talisman in Production
>
> Disabling Talisman (`TALISMAN_ENABLED = False`) removes critical HTTP security headers and leaves your deployment vulnerable to XSS, clickjacking, and protocol downgrade attacks. Only disable it if you are providing equivalent protections through an external service (e.g., a reverse proxy or WAF).
