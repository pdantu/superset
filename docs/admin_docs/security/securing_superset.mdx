...
> #### ⚠️ Important: Verify Talisman is Enabled
>
> `TALISMAN_ENABLED` defaults to `True`. If it is explicitly set to `False`, a warning will be logged at startup. You should ensure it remains enabled in your `superset_config.py` for the security headers defined in `TALISMAN_CONFIG` to take effect.
